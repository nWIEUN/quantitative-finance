...
6.	在上面的基础上对沪深300全体股票进行测试，观察因子表现效果，需要对行业和市值做中性化处理，避免收益率虚高（行业和市值的中性化主要集中在数据处理和因子的构建两方面的修改，数据方面通过线性回归，除去这两者的影响，主要用于噪声特征的提取，因子构建方面同样对因子值进行二者的线性回归，除去二者的影响，留下纯净的因子）。
